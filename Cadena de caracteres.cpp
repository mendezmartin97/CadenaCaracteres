...
template<typename T>//TIENE QUE DEVOLVER LA POSICION DONDE SE AGREGO
...
{
   Fecha f;
   f.dia = stringToInt(getTokenAt(s,'/',0));
   f.mes = stringToInt(getTokenAt(s,'/',1));
   f.anio = stringToInt(getTokenAt(s,'/',2));
   return f;
}
...
{
   int difA = f1.anio - f2.anio;
   int difM = f1.mes - f2.mes;
   int difD = f1.dia - f2.dia;
   if(difA!=0)
      return difA;
   else
   {
...
int diaCmp(Fecha f, int dia)
{
   return f.dia-dia;
}
int anioCmp(Fecha f,int anio)
{
   return f.anio-anio;
}
int mesCmp(Fecha f,int mes)
{
   return f.mes-mes;
}
int main() {
   /*
    * Para probar la funcion de collFind
    * le dejo el ejemplo de abajo, se puede buscar por dia
    * por mes o por anio.
    * */

   int pos,num;
   Coll c=collCreate('|');
   collAdd(c,fechaCreate(02,06,2017),fechaToString);
   collAdd(c,fechaCreate(10,02,1980),fechaToString);
   collAdd(c,fechaCreate(26,10,1997),fechaToString);

   cout<<c.s<<endl<<"Estas son las fechas guardadas en la coleccion"<<endl;

   cout<<"Ingrese la palabra mes, anio o dia para buscar:"<<endl;
   string s="";
   cin>>s;
   cout<<"ingrese el numero de "<<s<<endl;
   cin>>num;
   if(s=="mes")
      pos=collFind(c,num,mesCmp,stringToFecha);
   if(s=="anio")
      pos=collFind(c,num,anioCmp,stringToFecha);
   if(s=="dia")
      pos=collFind(c,num,diaCmp,stringToFecha);

   cout<<"Fecha en la posicion "<<pos<<" en la coleccion."<<endl;

   //Aca ordenamos la coleccion de menor a mayor

   collSort(c,fechaCmp,stringToFecha,fechaToString);
   cout<<c.s<<endl<<"Estas son las fechas ordenadas de menor a mayor";
   //Alumno Martin Mendez - K1001
	return 0;
}
